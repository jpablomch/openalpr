...
int main(int argc, char** argv) {
  ...
  for (unsigned int font_sheet_index = 0; font_sheet_index < font_sheet_files.size(); font_sheet_index++)
  {
    ...
    for (unsigned int t = 0; t < thresholds.size(); t++)
    {
      ...
      findContours(speckle_copy, speckle_contours, speckle_hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
      ...
      for (unsigned int i = 0; i < speckle_contours.size(); i++)
      {
        ...
        {
          drawContours(thresholds[t], speckle_contours, i, Scalar(0,0,0), cv::FILLED);
          drawContours(testImg, speckle_contours, i, Scalar(255,255,255), cv::FILLED);
        }
      }
      ...
      findContours(blobby, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
